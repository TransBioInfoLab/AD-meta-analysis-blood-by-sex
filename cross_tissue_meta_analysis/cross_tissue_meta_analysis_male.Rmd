...
## Paths
```{R}
dir.data.aux <- file.path("~/TBL Dropbox/Tiago Silva/AD-meta-analysis-blood-samples/datasets/Aux/") 
dir.results <- file.path("~/TBL Dropbox/Tiago Silva/AD-meta-analysis-blood-samples-bySex/analysis_results")
...
blood.meta_analysis.male <- readr::read_csv(
  "analysis_results/meta_analysis/Logistic_regression_model/AD_vs_CN/MALE_meta_analysis_glm_fixed_effect_ADNI_and_AIBL_AD_vs_CN_single_cpg_annotated.csv"
  )
...
blood_and_brain <- full_join(
  brain.meta_analysis.male[,c("cpg",grep("_",grep("bacon",colnames(brain.meta_analysis.male),value = TRUE),value = T))],
  blood.meta_analysis.male[,c("cpg",grep("_",grep("bacon",colnames(blood.meta_analysis.male),value = TRUE),value = T))]
)
```
...
n_rosmap <- 265
#n_london_blood <- 77
n_adni <- 165 # AD and CN
n_aibl <- 223 # AD and CN
...
meta.with.blood <- meta %>% dplyr::filter(! (is.na(meta$ADNI_MALE_pValue.bacon)  & is.na(meta$AIBL_MALE_pValue.bacon)))
...
meta.with.blood.annotated <- readr::read_csv(paste0(dir.result.cross.meta.analysis, prefix,"cross_tissue_meta_analysis_glm_using_AD_vs_CN_single_cpg_annotated.csv"))
p.values.brain <- cbind(
  brain.meta_analysis.male[,c("cpg","chr","start","GASPARONI_pValue.bacon","LONDON_pValue.bacon","MTSINAI_pValue.bacon","ROSMAP_pValue.bacon")],
  data.frame("brain_estimate_direction" = paste0(
    gsub("1","+",gsub("-1","-",sign(brain.meta_analysis.male$GASPARONI_Estimate.bacon))),
    gsub("1","+",gsub("-1","-",sign(brain.meta_analysis.male$LONDON_Estimate.bacon))),
    gsub("1","+",gsub("-1","-",sign(brain.meta_analysis.male$MTSINAI_Estimate.bacon))),
    gsub("1","+",gsub("-1","-",sign(brain.meta_analysis.male$ROSMAP_Estimate.bacon)))
  ))
)

p.values.blood <- cbind(
  blood.meta_analysis.male[,c("cpg","ADNI_MALE_pValue.bacon","AIBL_MALE_pValue.bacon","nasser_is_enahncer","nasser_enhancer_cell_type","GREAT_annotation")],
  data.frame("blood_estimate_direction" = paste0(
    gsub("1","+",gsub("-1","-",sign(blood.meta_analysis.male$ADNI_MALE_Estimate.bacon))),
    gsub("1","+",gsub("-1","-",sign(blood.meta_analysis.male$AIBL_MALE_Estimate.bacon)))
  ))
)
...
p.values.meta.analysis <- data.frame(
  ...
  "brain_meta_analysis_pval" = brain.meta_analysis.male$pVal.final[match(p.values.brain.blood$cpg,brain.meta_analysis.male$cpg)],
  "blood_meta_analysis_pval" = blood.meta_analysis.male$pVal.final.bacon[match(p.values.brain.blood$cpg,blood.meta_analysis.male$cpg)],
...
final.table <- final.table %>% dplyr::relocate(c("blood_estimate_direction"),.after = "AIBL_MALE_pValue.bacon")
final.table <- final.table %>% dplyr::relocate(c("blood_estimate_direction"),.after = "AIBL_MALE_pValue.bacon")
writexl::write_xlsx(
  x = final.table,
  path = "~/TBL Dropbox/Tiago Silva/AD-meta-analysis-blood-samples-bySex/analysis_results/cross_meta_analysis/MALE_all_meta_analysis.csv"
)
```
