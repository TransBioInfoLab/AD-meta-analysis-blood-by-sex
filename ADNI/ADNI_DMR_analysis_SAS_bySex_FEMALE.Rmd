...
adni.se <- adni.se[,adni.se$PTGENDER == "Female"]
...
save(
  coMeth_ls,
  probes.cluster.all, # all regions
  file = file.path(dir.data.dmr,"FEMALE_coMeth_ls_min_age_at_visit_65.rda")
)
```

# Take the median
```{R, eval = FALSE}
load(file.path(dir.data.dmr,"FEMALE_coMeth_ls_min_age_at_visit_65.rda"))
...
filename <-  file.path(dir.data.dmr,"medianMvals_and_pheno_min_age_at_visit_65_FEMALE.rda")
...
if(!file.exists(filename)){
  ...
  medianMval.df <- medianMval.df[rowSums(is.na(medianMval.df)) < ncol(medianMval.df),]
...
output.path <- "~/TBL Dropbox/Tiago Silva/AD-meta-analysis-blood-samples-bySex/analysis_results/ADNI/DMR_AD_CN/DATA_FEMALE/"
...
for(start in seq(1,nrow(medianMval.df),by = 1000)){
  ...
  readr::write_csv(
    final,
    paste0(output.path,"FEMALE_ADNI_AD_CN_1000_dmrs_",idx,".csv")
  )
}
...
files <- dir(
  path = file.path("~/TBL Dropbox/Tiago Silva/AD-meta-analysis-blood-samples-bySex/LW/Females_DMR_SAS/results/"),
  full.names = T,
  recursive = T,
...
readr::write_csv(glmm, file = file.path(dir.results.dmr,"FEMALE_AD_vs_CN_glmm_using_beta.cvs"))
```
...
results <- readr::read_csv(file.path(dir.results.dmr,"FEMALE_AD_vs_CN_glmm_using_beta.cvs"))
load(file.path(dir.data.dmr,"FEMALE_coMeth_ls_min_age_at_visit_65.rda"))
...
save(great, file = file.path(dir.data.aux,"FEMALE_great_coMethDMR_EPIC_array_annotation_ADNI.rda"))
```

## Add annotation to input cpgs
```{R annotate, eval = FALSE}
load(file.path(dir.data.aux,"FEMALE_great_coMethDMR_EPIC_array_annotation_ADNI.rda"))
...
readr::write_csv(
  ...
  file =  file.path(dir.results.dmr,"FEMALE_DMR_AD_vs_CN_results_annotated.csv")
)
...
readr::write_csv(
  ...
  file = file.path(dir.results.dmr, "FEMALE_DMR_AD_vs_CN_results_annotated_pvalue_cut_off_0_05.csv")
)
...
readr::write_csv(
  ...
  file = file.path(dir.results.dmr,"FEMALE_DMR_AD_vs_CN_results_annotated_with_inflation.csv")
)
```

```{R}
plot_inflation(logistic_mixed_model_ad_cn.with.inflation, "FEMALE ADNI\nDMR GLMM - AD vs CN")
```
...
sinlge.cpg.ad.vs.cn <- readr::read_csv( 
  file = file.path("~/TBL Dropbox/Tiago Silva/AD-meta-analysis-blood-samples-bySex/analysis_results/ADNI/Logistic_regression_model/Females_ADNI_AD_vs_CN_glmm_using_beta_with_inflation.csv"),
  col_types = readr::cols()
)
results.ad.annotated.sig <- readr::read_csv(
  file.path(dir.results.dmr, "FEMALE_DMR_AD_vs_CN_results_annotated_with_inflation.csv"),
  col_types = readr::cols()
)
