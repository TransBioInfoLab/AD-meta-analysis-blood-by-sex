---
title: "AIBL dataset MALE- DMR analysis"
...
se.male <- aibl.se.with.sex[,aibl.se.with.sex$Sex == "M"]
rm(aibl.se.with.sex)
gc()
```

```{R, eval = FALSE}
coMeth_ls.male <- CoMethAllRegions(
  dnam = assay(se.male),      
  betaToM = TRUE,                   
...
save(
  ...
  file = file.path(dir.data.dmr,"Male_coMeth_ls.rda")
)
...
```{R, eval = FALSE}
load(file.path(dir.data.dmr,"Male_coMeth_ls.rda"))
pheno <- data.frame(colData (se.male))
...
filename <-  file.path(dir.data.dmr,"medianBeta_and_pheno_male.rda")
beta_mat <- assay(se.male) %>% as.matrix()
...
if(!file.exists(filename)){
  medianBetaVal.df <- plyr::ldply(
    probes.cluster.all,
    function(probes){
      colMedians(beta_mat[rownames(beta_mat) %in% as.character(probes),, drop = F], na.rm = TRUE)
    },
    .progress = "time"
  )
...
readr::write_csv(results, file.path(dir.results.dmr,"MALE_DMR_results_all.csv"))
```

# Annotate results 

```{R, eval = FALSE}
results <- readr::read_csv(file.path(dir.results.dmr,"MALE_DMR_results_all.csv"))
load(file.path(dir.data.dmr,"Male_coMeth_ls.rda"))
...
save(great, file = file.path(dir.data.aux,"MALE_great_AIBL_coMethDMR_EPIC_array_annotation.rda"))
```

## Add annotation to input cpgs
```{R, eval = FALSE}
load(file.path(dir.data.aux,"MALE_great_AIBL_coMethDMR_EPIC_array_annotation.rda"))
...
readr::write_csv(
  ...
  file =  file.path(dir.results.dmr,"MALE_DMR_glm_AD_vs_CN_results_annotated.csv")
)
...
readr::write_csv(
  ...
  file = file.path(dir.results.dmr, "MALE_DMR_glm_AD_vs_CN_results_annotated_pvalue_cut_off_0_05.csv")
)
...
sinlge.cpg.ad.vs.cn <- readr::read_csv( 
  file.path("~/TBL Dropbox/Tiago Silva/AD-meta-analysis-blood-samples-bySex/analysis_results/AIBL/single_cpg/Logistic_regression_model/MALE_AIBL_glm_diagnosis_vs_beta_and_covaritates_age_at_least_65_AD_vs_CN_results_annotated_with_inflation.csv"),
  col_types = readr::cols()
)

results.ad.annotated.sig <- readr::read_csv(
  file.path(dir.results.dmr, "MALE_DMR_glm_AD_vs_CN_results_annotated_pvalue_cut_off_0_05.csv"),
  col_types = readr::cols()
)
...
readr::write_csv(
  ...
  file = file.path(dir.results.dmr,"MALE_DMR_glm_AD_vs_CN_results_annotated_with_inflation.csv")
)

```

```{R}
plot_inflation(results.ad.annotated.with.inflation, "MALE AIBL DMR GLMM - AD vs CN")
```
