...
results.files <- results.files[grep("FEMALE",results.files,invert = F)]
...
readr::write_csv(
  ...
  file = paste0(dir.result.meta.analysis.ad, "FEMALE_meta_analysis_glm_fixed_effect_ADNI_and_AIBL_AD_vs_CN_DMR.csv")
)
```
...
add_annotation <- function(result){
    ...
    result$max <- ifelse(result$start > result$end,result$start,result$end)
    result$min <- ifelse(result$start < result$end,result$start,result$end)
...
readr::write_csv(
  ...
  file = paste0(dir.result.meta.analysis.ad, "FEMALE_meta_analysis_glm_fixed_effect_ADNI_and_AIBL_AD_vs_CN_DMR_annotated.csv")
)
...
readr::write_csv(
  ...
  file = paste0(
    ...
    "FEMALE_meta_analysis_glm_fixed_effect_ADNI_and_AIBL_AD_vs_CN_DMR_annotated_pvalue_cut_off_0_05.csv"
  )
)
```
