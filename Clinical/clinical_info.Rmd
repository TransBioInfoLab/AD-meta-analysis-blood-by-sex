...
rbind(by.group[,-1],all) %>% gt() %>%
  tab_header(
    ...
    subtitle = paste0(
      "Patient cohort (N=",clinical %>% dplyr::filter(!is.na(DX)) %>% 
        dplyr::filter(DX != "MCI" & PTGENDER == "Female") %>%  dplyr::filter(Visit_number == 1) %>%  dplyr::pull(RID) %>% unique %>% length(),"; ",
      clinical %>% dplyr::filter(!is.na(DX)) %>% 
...
rbind(by.group[,-1],all) %>% gt() %>%
  tab_header(
    ...
    subtitle = paste0(
      "Patient cohort (N=",clinical %>% dplyr::filter(!is.na(DX)) %>% 
        dplyr::filter(DX != "MCI" & PTGENDER == "Male") %>%  dplyr::filter(Visit_number == 1) %>%  dplyr::pull(RID) %>% unique %>% length(),"; ",
      clinical %>% dplyr::filter(!is.na(DX)) %>% 
...
AddNeuroMed.se <- readRDS(
  file.path(dir.data,"AddNeuroMed/step3_pca_filtering/addNeuroMed_QNBMIQ_PCfiltered.RDS")
)
